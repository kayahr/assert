...
describe("assertThrow", () => {
    it("does not throw when synchronous function throws something", () => {
        assert.doesNotThrow(() => assertThrow(() => { throw new Error(); }));
    });
    ...
    it("does not throw when asynchronous function throws something", async () => {
        await assert.doesNotReject(() => assertThrow(() => Promise.reject(new Error())));
    });
    it("does not throw when asynchronous function throws expected error", async () => {
        await assert.doesNotReject(() => assertThrow(() => Promise.reject(new Error("2")), new Error("2")));
    });
    ...
    it("does throw when asynchronous function throws nothing", async () => {
        await assert.rejects(() => assertThrow(() => Promise.resolve()), new AssertionError("Expected function to throw"));
        await assert.rejects(() => assertThrow(() => Promise.resolve(), new Error("3")), new AssertionError("Expected function to throw <Error(Error: 3)>"));
    });
    it("does throw when asynchronous function does not throw expected error", async () => {
        await assert.rejects(() => assertThrow(() => Promise.reject(new Error("1")), new Error("2")),
